...
/**
 * Node that represents the binary comparison operator "greater than or equal
 * to".
 * 
 * @author pedroalves
 *
...
public class ASTGreaterEqual implements ASTNode {
	...
	public ASTGreaterEqual(final ASTNode left, final ASTNode right) {
		this.left = left;
		this.right = right;
	}
	...
	public Type typecheck(final TypeEnvironment tenv) throws TypeError, UnknownIdentifierException {
		...
		if (lt.getTypeDesc() == TypeDesc.INT && rt.getTypeDesc() == TypeDesc.INT) {
			...
			throw new TypeError("Undefined operator " + this + " for the arguments: " + lt + ", "
					+ rt);
